...
import { SafeApiError, verifyAction, type VerifyClientOptions } from '../src/client.js'
...
test('the released client type removes caller-selected origin', () => {
  const rejectedAtCompileTime: VerifyClientOptions = {
    apiKey: 'sentinel-key',
    // @ts-expect-error origin is intentionally absent from released options
    origin: 'http://untrusted.invalid',
  }
  assert.equal((rejectedAtCompileTime as unknown as Record<string, unknown>).origin, 'http://untrusted.invalid')
})
...
for (const hostileOrigin of ['http://untrusted.invalid', 'https://alternate.invalid']) {
  test(`untyped origin ${hostileOrigin} cannot alter the fixed HTTPS endpoint`, async () => {
    ...
    const options = { apiKey: 'sentinel-key', fetchImpl, origin: hostileOrigin } as unknown as VerifyClientOptions
...
test('rejects a redirect response without following or retrying', async () => {
  ...
  await assert.rejects(
    () => verifyAction(input, { apiKey: 'sentinel-key', fetchImpl }),
    (error: unknown) => error instanceof SafeApiError && error.status === 302,
  )
  assert.equal(calls, 1)
})
...
test('rejects an oversized serialized request before fetch', async () => {
  ...
  await assert.rejects(
    () => verifyAction(oversized, { apiKey: 'sentinel-key', fetchImpl }),
    (error: unknown) => error instanceof SafeApiError && error.status === 413 && error.code === 'payload_too_large',
  )
  assert.equal(calls, 0)
})

function oversizedStreamResponse(status: number, headers?: HeadersInit): Response {
  let sent = 0
...
test('rejects an oversized success stream with missing Content-Length before JSON parse', async () => {
  const fetchImpl: typeof fetch = async () => oversizedStreamResponse(200)
  await assert.rejects(
    () => verifyAction(input, { apiKey: 'sentinel-key', fetchImpl }),
...
test('rejects an oversized chunked error stream despite misleading Content-Length', async () => {
  const fetchImpl: typeof fetch = async () => oversizedStreamResponse(500, {
    'content-length': '7',
    'transfer-encoding': 'chunked',
  })
  await assert.rejects(
    () => verifyAction(input, { apiKey: 'sentinel-key', fetchImpl }),
    (error: unknown) => error instanceof SafeApiError && error.status === 500 && error.code === 'payload_too_large',
  )
})
...
test('allowlists a documented error code while discarding upstream strings', async () => {
  ...
  await assert.rejects(
    () => verifyAction(input, { apiKey: 'never-print-this', fetchImpl }),
    (error: unknown) => {
...
test('normalizes an undocumented error code to unknown', async () => {
  ...
  await assert.rejects(
    () => verifyAction(input, { apiKey: 'sentinel-key', fetchImpl }),
    (error: unknown) => error instanceof SafeApiError && error.code === 'unknown',
  )
})

test('returns a bounded error for invalid JSON without exposing the credential or body', async () => {
  const fetchImpl: typeof fetch = async () => new Response('raw-body-fixture', { status: 502 })
  await assert.rejects(
    () => verifyAction(input, { apiKey: 'never-print-this', fetchImpl }),
    (error: unknown) => {
      assert.ok(error instanceof SafeApiError)
...
test('aborts one in-flight request at the configured timeout', async () => {
  ...
  await assert.rejects(
    () => verifyAction(input, { apiKey: 'sentinel-key', fetchImpl, timeoutMs: 5 }),
